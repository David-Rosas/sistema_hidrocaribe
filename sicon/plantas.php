...
if(isset($_GET['id']))
{
	$id = $_GET['id'];
	$max_id = $id - 1;
	$sql = "SELECT nombre from plantas WHERE id='$id'";
	$nombre = resultadoIndividual($connection,$sql);
}
else
{
	$sql = "SELECT max(id) from plantas";
...
$cuerpo->set_titulo("SICON - PLANTA Y ESTACIONES DE AGUA SERVIDAS");
$cuerpo->set_languaje();
...
$cuerpo->abrir_body('align="center"',' action="index1.php" method="post" name="form"',"40%");
	$cuerpo->esp_parrafo();
	$marco->set_class_titulo('tituloDOSE_4');
	$marco->set_colspan(2);
	$marco->set_propiedades('class="formato-blanco" align="center" border="0" cellpadding="0" cellspacing="0" width="50%"');
	$marco->set_titulo("");
	$marco->abrir_marco();
