...
$sql = "SELECT max(id) from operador";
$max_id = resultadoIndividual($connection,$sql);
$cuerpo->set_titulo("SICOCON - BUSQUEDA DE OPERADORAS");
$cuerpo->set_languaje();
...
$cuerpo->abrir_head();
	...
	$cuerpo->add_script($dir_javascript."operadora.js");
$cuerpo->cerrar_head();

$cuerpo->abrir_body('align="center"',' action="operadora_buscar.php" method="get" name="form"',"70%");
	$cuerpo->esp_parrafo();
	...
	$marco->abrir_marco();
		...
		$editar = new boton($dir_png."editar.png","editar",$title="Editar Operadora");
		$eliminar = new boton($dir_gif."eliminar.gif","eliminar",$title="Eliminar Operadora");		
		
		$tabla = new grid($connection,"id,nombre","operador",array("Codigo","Nombre"),array($editar,$eliminar),"nombre ilike '%$nombre%' and eliminado=0","nombre");
