...
if(isset($_GET['id']))
{
	...
	$sql = "SELECT sitio from maniobras WHERE id='$id'";
...
$control->set_value($datos[0]);
$control->set_propiedades('style="text-align: left;" onchange="operadora_contrato();"');
$marco->add_control($control, $connection,"SELECT id,nombre FROM valvula where eliminado=0 order by nombre",1);

$control->set_propiedades('style="text-align: left;" readonly="readonly" id="op"');
$control->set_label("ubicacion");
//$control->set_name("id_operadora");
$control->set_name("idcatv");
$marco->add_control($control, $connection,"SELECT id,nombre FROM catvalvula where eliminado=20 order by nombre",1);
