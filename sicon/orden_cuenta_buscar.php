...
$cuerpo->set_titulo("SICOCON - BUSQUEDA DE ORDENES DE TRABAJO CON PUNTO DE CUENTA");
$cuerpo->set_languaje();
...
$cuerpo->abrir_head();
	...
	$cuerpo->add_script($dir_javascript."orden_cuenta_buscar.js");
$cuerpo->cerrar_head();
...
$cuerpo->abrir_body('align="center"',' action="actividad_buscar.php" method="get" name="form"',"90%");
	...
	$marco->abrir_marco();
		...
		$tabla = new grid($connection,"id,fecha,operadora,actividad,descripcion,nombre,direccion","orden_cuenta",array("id","Fecha","Operadora","Actividad","Descripcion","Cliente","Dirección"),array($listo,$editar,$imprimir),"(id ilike '%$datos%' or fecha ilike '%$datos%' or contrato ilike '%$datos%' or operadora ilike '%$datos%' or actividad ilike '%$datos%' or descripcion ilike '%$datos%' or punto_cuenta ilike '%$datos%' or nombre ilike '%$datos%' or direccion ilike '%$datos%' or tlf ilike '%$datos%')","fecha desc,hora desc limit 20");
