...
if(isset($_GET['id']))
{
	$id = $_GET['id'];
	$id_max = $id - 1;
	$sql = "SELECT id_contrato,id_operadora,id_actividad,descripcion,punto_cuenta,nombre,direccion,tlf,id_cliente 
from orden_cuenta 
WHERE id='$id'";
...
$cuerpo->add_script($dir_javascript."orden_cuenta.js");
...
//$suscriptor->add_control($control);
	...
	$control->set_label("Punto de Cuenta");
	$control->set_name("cuenta");
	$control->set_value($datos[4]);
$suscriptor->add_control($control);

$escojer = new boton($dir_gif."buscar.gif","cliente",$title="Buscar un Cliente");
$control->set_boton($escojer);
$control->set_label("Código Cliente");
$control->set_value($datos[8]);
$control->set_name("id_cliente");
$control->set_propiedades('style="text-align: left;"');
$suscriptor->add_control($control);
$control->set_propiedades('style="text-align: left;"');
...
$control->set_label("Direccion");
	$control->set_name("direccion");
	$control->set_value($datos[6]);
$suscriptor->add_control($control);

$control->set_label("Telefono");
	$control->set_name("tlf");
	$control->set_value($datos[7]);
$suscriptor->add_control($control);
