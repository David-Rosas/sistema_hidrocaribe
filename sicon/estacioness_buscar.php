...
$cuerpo->abrir_body('align="center"',' action="estacioness_buscar.php" method="post" name="form"',"70%");
	$cuerpo->esp_parrafo();
	...
	$marco_busqueda->abrir_marco();
		...
		$control->set_propiedades('style="text-align: left;"' );
		$marco->add_control($control, $connection,"SELECT nombre FROM plantas where eliminado=0 order by nombre");
...
//	    $c6->set_value("Grafica");
	...
	$marco->abrir_marco();
		...
		$tabla = new grid1($connection,"id,fecha,hora,sitio,causa,causa2,observacion,fechaa,horaa,atendido","estacioness",array("Codigo","Fecha","Hora","Planta","Causa","Categoria","Observación","Fecha A","Hora Arranque","Atendido"),array($editar),"fecha ilike '%$fecha%' and eliminado=0","fecha");
