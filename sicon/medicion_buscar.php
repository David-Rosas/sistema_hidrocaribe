...
$sql = "SELECT max(id) from medicion";
$max_id = resultadoIndividual($connection,$sql);
$cuerpo->set_titulo("SICON - BUSQUEDA DE PUNTO DE MEDICION");
$cuerpo->set_languaje();
...
$cuerpo->abrir_head();
	...
	$cuerpo->add_script($dir_javascript."medicion.js");
$cuerpo->cerrar_head();

$cuerpo->abrir_body('align="center"',' action="medicion_buscar.php" method="get" name="form"',"70%");
	$cuerpo->esp_parrafo();
	...
	$marco->set_titulo("Tipo de Contrato");
	...
	$marco->abrir_marco();
		...
		$editar = new boton($dir_png."editar.png","editar",$title="Editar Tipo de Contrato");
		$eliminar = new boton($dir_gif."eliminar.gif","eliminar",$title="Eliminar Contrato");		
		
		$tabla = new grid($connection,"id,nombre","medicion",array("Codigo","Nombre"),array($editar,$eliminar),"nombre ilike '%$nombre%' and eliminado=0","nombre");
