...
function generar_pdf(servidor,id)
{
	...
	{  
		...
		if(confirm("Desea que SICOCON imprima la Orden de Trabajo ??"))
			location.href="orden_cuenta_pdf.php?id="+id;	  
   } 
}
function guardar()
{
	url='orden_cuenta_insert.php?cuenta='+document.form.cuenta.value+'&nombre='+document.form.nombre.value+'&id='+document.form.id.value+'&direccion='+document.form.direccion.value+'&tlf='+document.form.tlf.value+'&id_contrato='+document.form.id_contrato.value+'&id_operadora='+document.form.id_operadora.value+'&id_actividad='+document.form.id_actividad.value+'&descripcion='+document.form.descripcion.value+'&id_cliente='+document.form.id_cliente.value+'&ale='+aleatorio(0,999999);
	//alert(url);
...
function cliente(id,url)
{
	window.open('buscar_cliente.php','toolbar=0,location=0,directories=0,status=1,menubar=0,scrollbars=1,resizable=1,width=550,height=800');
}
function nuevo()
{
	if(confirm('Esta Seguro que desea Limpiar la Ventana?.\nSe Pueden Perder Datos!!'))
	{
		location.href="orden_cuenta.php";
	}
}