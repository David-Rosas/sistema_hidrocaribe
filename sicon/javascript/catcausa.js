...
{
	location.href="ctacausa.php?id="+id+"&ale="+aleatorio(0,999999);
}
function eliminar(id,remitente)
{
	if(confirm("Esta seguro(a) que desea Eliminar la informacion id: "+id+" ??"))
		{
			new Ajax.Request('catcausa_delete.php?id='+id+"&ale="+aleatorio(0,999999), 
   		{
   			method:'get',
...
function find()
{
	location.href="catcausa_buscar.php?nombre="+document.form.nombre.value+'&ale='+aleatorio(0,999999);
}
function guardar()
{
	if(vacio(document.form.nombre.value,"Nombre"))
	{
		if(confirm("Esta seguro(a) de Almacenar los Siguientes Datos?? \n ID: "+document.form.id.value+"\n NOMBRE: "+document.form.nombre.value+"\n Causa: "+document.form.causa.value))
		{
			new Ajax.Request('catcausa_insert.php?id='+document.form.id.value+'&nombre='+document.form.nombre.value+'&causa='+document.form.causa.value+'&ale='+aleatorio(0,999999), 
   		{
   			method:'get',
   			onSuccess: function(transport)
   						{
