...
{
	location.href="planta.php?id="+id+"&ale="+aleatorio(0,999999);
}
function eliminar(id,remitente)
{
	if(confirm("Esta seguro(a) que desea Eliminar la informacion id: "+id+" ??"))
		{
			new Ajax.Request('planta_delete.php?id='+id+"&ale="+aleatorio(0,999999), 
   		{
   			method:'get',
...
function find()
{
	location.href="planta_buscar.php?nombre="+document.form.nombre.value+'&ale='+aleatorio(0,999999);
}
function guardar()
{
	if(vacio(document.form.nombre.value,"Nombre"))
	{
		...
		{
			new Ajax.Request('planta_insert.php?id='+document.form.id.value+'&nombre='+document.form.nombre.value+'&ale='+aleatorio(0,999999), 
   		{
   			method:'get',
   			onSuccess: function(transport)
   						{
