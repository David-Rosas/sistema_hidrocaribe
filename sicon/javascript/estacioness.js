...
{
	location.href="estacioness.php?id="+id+"&ale="+aleatorio(0,999999);
}
...
function eliminar(id,remitente)
{
	if(confirm("Esta seguro(a) que desea Eliminar la informacion de la Operadora numero: "+id+" ??"))
		{
			new Ajax.Request('estacioness_delete.php?id='+id+"&ale="+aleatorio(0,999999), 
   		{
   			method:'get',
...
function find()
{
	location.href="estacioness_buscar.php?fecha="+document.form.fecha.value+'&ale='+aleatorio(0,999999);
}
function guardar()
{
...
function remoto()
{
	var url = "http://localhost/sicon/estacioness_insert.php";
	//alert(url);
