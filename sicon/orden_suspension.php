...
if(isset($_GET['id']))
{
	$id = $_GET['id'];
	$id_max = $id - 1;
	$sql = "SELECT id,fecha,punto,hora,presion,caudal from tomas WHERE id='$id'";
...
$cuerpo->add_script($dir_javascript."orden_suspension.js");
...
//$suscriptor->add_control($control);
	...
	$control->set_label("Listado de Suspensiones");
...
$marco->add_control($control, $connection,"SELECT id,fecha,punto,hora,presion,caudal FROM tomas where eliminado=0 order by nombre",1);
...
$marco->add_control($control, $connection,"SELECT id,fecha,punto,hora,presion,caudal FROM tomas where eliminado=20 order by nombre",1);
