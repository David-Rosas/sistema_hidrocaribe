...
$cuerpo->set_titulo("SICOCON - BUSQUEDA DE ORDENES DE TRABAJO CON LISTADO DE SUSPENSIONES SIN FINALIZAR");
$cuerpo->set_languaje();
...
$cuerpo->abrir_head();
	...
	$cuerpo->add_script($dir_javascript."orden_suspension_buscar.js");
$cuerpo->cerrar_head();
...
$cuerpo->abrir_body('align="center"',' action="actividad_buscar.php" method="get" name="form"',"90%");
	...
	$marco->abrir_marco();
		...
		$editar = new boton($dir_png."editar.png","editar",$title="Editar Tipo de Orden de Trabajo");
		$listo = new boton($dir_gif."listo.gif","listo",$title="Confirmar Ejecución del Trabajo");
		$imprimir = new boton($dir_png."imprimir.png","imprimir",$title="Imprimir Orden de Trabajo");
						
		$tabla = new grid($connection,"id,fecha,operadora,actividad,descripcion,zonas,rutas,listado","ordenes",array("id","fecha","operadora","actividad","descripcion","zonas","rutas","listado"),array($listo,$editar,$imprimir),"(id ilike '%$datos%' or fecha ilike '%$datos%' or contrato ilike '%$datos%' or operadora ilike '%$datos%' or actividad ilike '%$datos%' or descripcion ilike '%$datos%' or zonas ilike '%$datos%' or rutas ilike '%$datos%' or listado ilike '%$datos%') and tipo=0","fecha desc,hora desc limit 20");
