...
if(isset($_GET['id']))
{
	$id = $_GET['id'];
	$max_id = $id - 1;
	$sql = "SELECT fecha from estacioness WHERE id='$id'";
	$fecha = resultadoIndividual($connection,$sql);
	$sql = "SELECT hora from estacioness WHERE id='$id'";
	$hora = resultadoIndividual($connection,$sql);
	$sql = "SELECT sitio from estacioness WHERE id='$id'";
	$sitio = resultadoIndividual($connection,$sql);
	$sql = "SELECT causa from estacioness WHERE id='$id'";
	$idc = resultadoIndividual($connection,$sql);
	$sql = "SELECT causa2 from estacioness WHERE id='$id'";
	$idc2 = resultadoIndividual($connection,$sql);
	$sql = "SELECT observacion from estacioness WHERE id='$id'";
	$observacion = resultadoIndividual($connection,$sql);
	$sql = "SELECT fechaa from estacioness WHERE id='$id'";
	$fechaa = resultadoIndividual($connection,$sql);
	$sql = "SELECT horaa from estacioness WHERE id='$id'";
	$horaa = resultadoIndividual($connection,$sql);
	$sql = "SELECT atendido from estacioness WHERE id='$id'";
	$atendido = resultadoIndividual($connection,$sql);
}
else
{
	$sql = "SELECT max(id) from estacioness";
...
$cuerpo->abrir_head();
	...
	$cuerpo->add_script($dir_javascript."estacioness.js");	
...
$cuerpo->abrir_body('',' action="index1.php" method="post" name="form"',"60%");
	...
	$marco->abrir_marco();
		...
		$control->set_value($idc);
		$marco->add_control($control);
		
		$control->set_propiedades('style="text-align: left;"');
		$control->set_label("CAUSA");
		$control->set_name("idc2");
		$control->set_value($idc2);
		$marco->add_control($control);
