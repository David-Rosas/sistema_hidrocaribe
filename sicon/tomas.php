...
if(isset($_GET['id']))
{
	$id = $_GET['id'];
	$max_id = $id - 1;
	$sql = "SELECT fecha from tomas WHERE id='$id'";
...
$cuerpo->abrir_head();
	...
	$cuerpo->add_script($dir_javascript."tomas.js");
...
$cuerpo->abrir_body('',' action="index1.php" method="post" name="form"',"60%");
	...
	$marco->abrir_marco();
		...
		$control->set_propiedades('style="text-align: left;"');
		$control->set_label("CAUDAL");
		$control->set_name("caudal");
		$control->set_value($caudal);
		$marco->add_control($control);
